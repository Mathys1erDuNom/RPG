...
from combat_image import creer_image_combat, creer_image_fin  # créer une image finale
...
class CombatView(View):
    def __init__(self, nb_ennemis=3):
        ...
        all_enemies = load_json("json/ennemies.json")  # liste de tous les ennemis

        # Sélection aléatoire ou séquentielle des ennemis à affronter
    ...
    async def joueur_attaque(self, interaction: discord.Interaction):
        ...
        if self.ennemi["pv"] <= 0:
            if self.ennemis_queue:
                ...
                image_fin = creer_image_fin(self.joueur)
                file = discord.File(fp=image_fin, filename="fin.png")
                await interaction.message.edit(
